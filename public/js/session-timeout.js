...
if (!isAuthenticated) {
  // Clear ONLY app keys (don’t nuke all localStorage)
  [
    'slea_last_activity',
    'slea_idle_deadline',
    'slea_warning_shown',
...
class SessionTimeout {
  ...
  handleActivity() {
    ...
    // If warning already shown, do not reset timers unless user explicitly clicks "Stay"
  ...
  showWarning() {
    ...
    // If tab hidden & notifications granted (fire only once per cycle)
    if (
      document.hidden &&
      !this.notified &&
    ...
    // timeoutTimer is already running
  }
  ...
  async sendKeepAlive() {
    try {
      ...
      // Don’t instantly logout on transient network hiccups
  ...
  async checkSessionStatus() {
    ...
    try {
      ...
      // Don’t instantly logout on transient failures
  ...
  handleTimeout() {
    ...
    // HARD STOP everything to avoid double triggers
    if (this.warningTimer) clearTimeout(this.warningTimer);
    if (this.timeoutTimer) clearTimeout(this.timeoutTimer);
