/* assessor_profile.js
   Scoped to Assessor Profile page (T-layout).
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showToast(message, isError = false, timeout = 3500) {
    ...
    toast.className = `assessor-toast ${isError ? 'assessor-toast-error' : 'assessor-toast-success'}`;
  ...
  function confirmModal(htmlBody) {
    ...
    return new Promise(resolve => {
      ...
      wrapper.innerHTML = `
      <div class="assessor-confirm-modal-backdrop" style="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.45);display:flex;align-items:center;justify-content:center;">
        <div class="assessor-confirm-modal" style="background:#fff;border-radius:8px;max-width:420px;width:92%;padding:1rem;box-shadow:0 8px 30px rgba(0,0,0,.2);">
          <div class="assessor-confirm-body">${htmlBody}</div>
          <div style="display:flex;gap:.6rem;justify-content:center;margin-top:.8rem;">
            <button class="assessor-confirm-cancel" style="padding:.5rem .9rem;border-radius:6px;border:1px solid #dcdcdc;background:#fff;">Cancel</button>
            <button class="assessor-confirm-ok" style="padding:.5rem .9rem;border-radius:6px;border:0;background:#0d6efd;color:#fff;">Confirm</button>
          </div>
        </div>
      </div>`;
      document.body.appendChild(wrapper);
      const cancelBtn = wrapper.querySelector('.assessor-confirm-cancel');
      const okBtn = wrapper.querySelector('.assessor-confirm-ok');
  ...
  function enableEditing() {
    ...
    // Make inputs editable except readonly ones (assessor id, email)
    updateForm.querySelectorAll('input').forEach(i => {
      if (i.hasAttribute('data-noreadonly') || i.getAttribute('name') === 'assessor_id' || i.readOnly) return;
      i.readOnly = false;
    });
  }
...
document.getElementById('cancelPasswordBtn')?.addEventListener('click', cancelPasswordEdit);
  ...
  function setChecklistState(key, ok) {
    ...
    if (checkIcon && circleIcon) {
      ...
      if (ok) {
        if (!el.querySelector('.assessor-check-mark')) {
          el.insertAdjacentHTML('afterbegin', '<span class="assessor-check-mark" style="display:inline-block;width:18px;margin-right:.45rem;color:#28a745;">✓</span>');
        }
        // remove any circle if present
        const circ = el.querySelector('.assessor-circle-mark');
        if (circ) circ.remove();
      } else {
        // remove check if present, ensure circle exists
        const chk = el.querySelector('.assessor-check-mark');
        if (chk) chk.remove();
        if (!el.querySelector('.assessor-circle-mark')) {
          el.insertAdjacentHTML('afterbegin', '<span class="assessor-circle-mark" style="display:inline-block;width:18px;margin-right:.45rem;color:#9aa0a6;">○</span>');
        }
      }
    }
  }
  ...
  async function handleAvatarFile(file) {
    ...
    reader.onload = async (ev) => {
      ...
      if (!action) {
        // fallback guess: use current location + 'assessor/profile/avatar' - but warn
