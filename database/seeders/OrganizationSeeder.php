...
class OrganizationSeeder extends Seeder
{
    ...
    {
        $clusters = Cluster::pluck('id', 'name')->toArray();

        if (empty($clusters)) {
            $this->command->warn('⚠️ No clusters found. Please run ClusterSeeder first.');
            return;
        }
        ...
        foreach ($organizationGroups as $clusterName => $orgs) {
            $clusterId = $clusters[$clusterName] ?? null;

            if (!$clusterId) {
                $this->command->warn("⚠️ Cluster not found: {$clusterName}");
                continue;
            }
        ...
        $this->command->info("✅ Seeded or updated {$total} organizations successfully!");
    }
}
